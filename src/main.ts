...
const waitForElementByXpath = (xpath: string): Promise<HTMLElement> => {
    return new Promise((resolve) => {
        if (document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue) {
            return resolve(document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue as HTMLElement);
        }
        const observer = new MutationObserver(() => {
            if (document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue) {
                observer.disconnect();
                resolve(document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue as HTMLElement);
            }
        });
...
const bindKeys = async () => {
    document.addEventListener('keydown', async (e) => {
        ...
        if (e.key === 'ArrowLeft') {
        ...
        else if (e.key === '1') { // number 1 to place order
            const trade = await waitForElementByXpath('//button[text()="Trade"]');
            trade.click();
        }
        else if (e.key === '2') { // number 2 to place order
            ...
            const [element, elType] = await Promise.race([
                waitForElementByXpath('//div[text()="+ Add"]').then(el => [el, 'tpSl']),
                waitForElementByXpath('(//*[starts-with(@id,"tabs")]//*[local-name() = "svg"])[2]').then(el => [el, 'edit'])
            ]);
            if (elType === 'tpSl') {
                (element as HTMLElement).click();
            } else {
                (element as HTMLElement).dispatchEvent(new Event('click', { bubbles: true }));
            }
            const input = await waitForElementByXpath('(//*[starts-with(@id,"dialog")]//input)[2]') as HTMLInputElement;
        ...
        else if (e.key === ' ') { // space to sell market
